...
public class CategoryHandler implements WooHandler {

    private UriBuilderSingleton uriBuilder;
    private Volley volley;


    public CategoryHandler(UriBuilderSingleton uriBuilder, Volley volley) {
    ...
    public void get(long id, OnResponse onResponse) {
        String url = uriBuilder.getCategory(id).build().toString();
        volley.basicAuthJsonObjReq(Request.Method.GET, url, null, new OnGetJsonObjectFinished() {
            @Override
            public void onSuccess(JSONObject response) {
                try {
                    if (onResponse != null)
                        onResponse.onSuccess(CategoryJsonConverter.jsonToCategory(response));
    ...
    public void getList(ParamBuilder builder, OnResponse onResponse) {
        String url = builder.buildParam(uriBuilder.getCategories());
