...
public class OrderHandler implements WooHandler {

    private UriBuilderSingleton uriBuilder;
    private Volley volley;


    public OrderHandler(UriBuilderSingleton uriBuilder, Volley volley) {
    ...
    public void get(long id, OnResponse onResponse) {
        String url = uriBuilder.getOrder(id).build().toString();
        volley.basicAuthJsonObjReq(Request.Method.GET, url, null, new OnGetJsonObjectFinished() {
            @Override
            public void onSuccess(JSONObject response) {
                try {
                    if (onResponse != null)
                        onResponse.onSuccess(OrderJsonConverter.jsonToOrder(response));
    ...
    public void getList(ParamBuilder builder, OnResponse onResponse) {
        String url = builder.buildParam(uriBuilder.getOrders());
